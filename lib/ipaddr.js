// IPv4 part pattern reused by IPv6 transitional regexes only.
// Loose / inet_aton IPv4 parsing is regex-free (see parseIPv4Aton below).
// Note that an address like 0010.0xa5.1.1 is considered legal for loose IPv4.
...
// Strict four-part decimal scanner (no leading zeros, octets 0-255).
// Equivalent accept set to isValid + /^(0|[1-9]\d*)(\.(0|[1-9]\d*)){3}$/.
function scanIPv4FourPart (string) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (v === 0) {
            if (i < 3) {
                if (p >= n || string.charCodeAt(p) !== 0x2e) {
                    return null;
                }
            } else if (p !== n) {
                return null;
        ...
        // Accumulate up to 2 more digits; reject a 4th digit before overflow math
        while (p < n) {
            const c = string.charCodeAt(p);
            if (c < 0x30 || c > 0x39) {
                break;
            }
            if (ndigits >= 3) {
...
function parseIntAutoDigits (string, start, end) {
    if (start >= end) {
        throw new Error('ipaddr: empty digit span');
    }
    const len = end - start;
    // leading 0 and a following digit → octal path (legacy parseIntAuto)
    if (string.charCodeAt(start) === 0x30 && len >= 2) {
        let v = 0;
        for (let k = start; k < end; k++) {
            const c = string.charCodeAt(k);
            if (c > 0x37) {
                // 8 or 9 — same throw as legacy octal parse
                throw new Error(`ipaddr: cannot parse ${string.slice(start, end)} as octal`);
            }
            v = v * 8 + (c - 0x30);
        }
        return v;
    ...
    let v = 0;
    for (let k = start; k < end; k++) {
        v = v * 10 + (string.charCodeAt(k) - 0x30);
    }
    return v;
}

// Same 1/2/3/4-field expansion + range checks as the legacy IPv4.parser.
function packIPv4Fields (vals) {
...
// Regex-free loose / inet_aton scanner — identical accept set to the
// former ipv4Regexes + parseIntAuto IPv4.parser path.
function parseIPv4Aton (string) {
    const n = string.length;
    if (n === 0) {
        return null;
    }
    ...
    while (i < n) {
        ...
        if (
            c0 === 0x30 &&
            i + 1 < n &&
            (string.charCodeAt(i + 1) === 0x78 || string.charCodeAt(i + 1) === 0x58)
        ) {
            i += 2;
            let v = 0;
            let digits = 0;
    ...
    return packIPv4Fields(vals);
}

// Regex-free CIDR split: last '/', non-empty addr, non-empty decimal mask digits.
function splitIPv4CIDR (string) {
    const n = string.length;
    let slash = -1;
    for (let i = n - 1; i >= 0; i--) {
    ...
    return { addr: string.slice(0, slash), mask: mask };
}
...
export class IPv4 {
    ...
    static isValidFourPartDecimal(string) {
        return scanIPv4FourPart(string) !== null;
    };

    // Checks if a given string is a full four-part IPv4 Address with CIDR prefix.
    static isValidCIDRFourPartDecimal(string) {
        const parts = splitIPv4CIDR(string);

        if (!IPv4.isValidCIDR(string) || !parts) {
            return false;
        }

        return IPv4.isValidFourPartDecimal(parts.addr);
    };
